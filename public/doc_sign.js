...
const S_submit =    document.getElementById("signButton");
const S_confirmPassword =  document.getElementById("passwordInput");

S_submit.addEventListener('click',registerDoctor);
function registerDoctor() {
     ...
     if(! Validation()){
      ...
      const dbRef = ref(db);
      get(child(dbRef, "DoctorList/" +S_username.value))
          .then((snapshot) => {
              if (snapshot.exists()) {
                  alert("User already exits");
              } else {
                  set(ref(db, "DoctorList/" + S_username.value),
                      {
